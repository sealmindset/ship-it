...
The caller workflow generated by the skill references the shared reusable workflow at `SleepNumberInc/sleep-number-claude-code-plugins/.github/workflows/ship-it-pipeline.yml@main`. The reusable workflow handles stack detection, build, lint, scan, and deploy. The skill does NOT generate build/deploy commands.
